...
# Emit the Zig source list build.zig imports. Generating it from the same *_C
# lists that drive the copy above means the compiled set and the vendored files
# can't drift: one bump regenerates both. build.zig has no hand-typed list.
{
  echo "// Generated by scripts/vendor-libwebp.sh — do not edit by hand."
  echo "// libwebp decoder translation units (LIBWEBPDECODER_OBJS), relative to"
  echo "// src/c/webp/src. build.zig imports this so the compiled file list and the"
  echo "// vendored sources share one source of truth: a bump regenerates both."
  echo "pub const files = [_][]const u8{"
  for f in ${DEC_C};       do echo "    \"dec/${f}.c\","; done
  for f in ${DSP_DEC_C};   do echo "    \"dsp/${f}.c\","; done
  for f in ${UTILS_DEC_C}; do echo "    \"utils/${f}.c\","; done
  echo "};"
} > "${DEST}/decoder_srcs.zig"

...
- The metadata block (`Episodes` / `Format` above the synopsis hairline) is the
  ...
  and a rail-only Pinned, ROD-345, all shipped; Provider and Pinned also
  surface on their own dedicated row in the compact form, §5.3a compact-line
  addendum, so both are live on this always-compact Browse-origin surface too);
...
highest-priority field first: **Episodes**, **Format**, **Source**,
**Duration**, **Studios**, **Rank** (ROD-261 widens the list to these six; see
below), then a rail-only **Provider** (ROD-348/356) and a rail-only **Pinned**
(ROD-345), appended in that order: Provider only when the open show carries a
canonical identity, Pinned only when it also carries a per-show provider pin.
Neither is AniList enrichment like the six fields ahead of them: Provider
reads `provider_absences` and the `anime` binding rows plus the live session's
...
Provider and Pinned also surface in the compact (non-bloomed) form, but not as
segments of this `·`-joined line: they render on a separate dedicated row
beneath it instead. See the compact-line addendum after the Pinned addendum
for the grammar and why (§8 also logs the call).
`drawHeader` (`view/detail.zig`) renders the field list through one of two
...
before Pinned by `selection.detailMetaFields`. `rail_only` within this shared
list (`drawMetaLine` never renders it as a joined segment); it also surfaces
in the compact form on its own dedicated row, covered in the compact-line
...
**ROD-345 addendum: Pinned.** The eighth field, appended after Provider by
...
outright when the show is unpinned: presence or absence, not a §9.1
enrichment degrade; there is no "still fetching" state for a pin. Nav-state
only: `detailMetaFieldsFor` (fed an explicit record, e.g. the History list
preview) never appends it, because the cached `App.show_pin` belongs to the
currently *focused* grid, not a cursor row a preview might be scrolled past.
Pinned stays the lowest-priority field (first to shed) because it is pure
per-show user override; Provider ranks one step above it because it reflects
real DB/session state rather than a manual choice, but both still sit below
the AniList enrichment sextet, which describes the show itself.

**Compact-line addendum (ROD-348/356 follow-up): Provider and Pinned get their
own dedicated row.** A live smoke test caught the gap this closes: below
`detail_two_col_min` the rail never blooms, so a `rail_only` Provider and
Pinned were invisible on every compact-width detail pane, exactly the width
most terminals run at day to day. A first attempt folded both into the
`·`-joined meta line itself (reordering it so Provider/Pinned sat ahead of
Duration/Studios); that was rejected on sight, this is a different category
of fact (routing/session state) from the AniList enrichment the joined line
otherwise carries, and cramming it into the same line as Duration/Studios
muddied both. The `MetaField` list, its order, and every field's `rail_only`
flag revert to exactly the original ROD-348/356/345 shape above: Episodes,
Format, Source, Duration, Studios, Rank, Provider, Pinned, with Provider and
Pinned `rail_only`.

The shipped fix lives entirely in the compact path. In the non-bloomed form
only, `drawHeader` draws one additional row directly beneath `drawMetaLine`'s
output, before the synopsis hairline: a new `drawProviderLine`, which is not
part of the generic field-list iteration either renderer uses. It finds the
already-computed Provider and Pinned entries in the same `fields` slice (a
...
Because this row is bespoke rather than a generic per-field print loop, the
`pin ` marker is a literal composed directly in `drawProviderLine`, not a
`MetaField`-level mechanism; a `prefix` field considered for the rejected
joined-line attempt has no consumer once that attempt is reverted and stays
cut. `MetaField` is exactly `{label, value, unit, dim, rail_only}`, unchanged
from before this whole addendum (§8 logs the call).

Omission: the whole row is skipped, no row consumed, when the show has no
canonical identity, the same gate Provider itself already uses. Placement is
fixed directly under the meta line and above the hairline into synopsis, not
adjacent to the score/genre line above the hairline, so no interaction there.
A height-starved pane drops this row the same way every other single
conditional row in `drawHeader` already does (`if (row < h)`); no new shed
mechanism, since this row sits outside the rail's own height-shed loop
entirely.
...
- The `28 eps · TV` row is the ROD-260/261 metadata grammar's compact-line form
  ...
  Episodes/Format (ROD-261, §5.3a/§5.3b), as has a rail-only Provider
  (ROD-348/356, the per-provider availability + serving line) and a rail-only
  Pinned (ROD-345, shown only when the show is pinned); both also surface on
...
the same 160 cols still renders the compact `28 eps · TV` line, plus its own
dedicated Provider/Pinned row below it (§5.3a compact-line addendum). Source,
Duration, Studios, and a rail-only Rank have shipped in the same rail
(§5.3a/§5.3b, ROD-261), a rail-only Provider (ROD-348/356) and a rail-only
Pinned (ROD-345) have shipped alongside them (Pinned shown only when the show
is pinned), and the airing countdown has shipped on the chips row instead
(§4.4); this mock
...
| **Detail · metadata line/rail** | AllAnime `eps_sub` / `eps_dub` and `kind`; AniList `total_episodes` fills if null; AniList `source` / `duration` / `studios` / `rankings` (ROD-261, shipped); DB `provider_pins` (ROD-345, per-show pin, shipped); DB `anime` bindings + `provider_absences` + live `episodes.for_source` (ROD-348/356, per-provider availability + serving, shipped) | `App.detailMetaFields()` (§5.3a, ROD-260/261/345/348/356) emits an ordered field list (Episodes, Format, Source, Duration, Studios, Rank, then a rail-only Provider, then a rail-only Pinned), rendered as the compact `N eps · kind · …` line (single-column surfaces, Rank/Provider/Pinned excluded) or the `Label  Value` rail (two-column surfaces, all eight); the compact form also draws a dedicated Provider/Pinned row of its own beneath the joined line (§5.3a compact-line addendum) | Episodes is the floor: `? eps` / `Episodes  ?` in `[d]` when neither source has a count, never omitted, so neither form is ever empty. Format/Source/Duration/Studios/Rank each omit outright when their field is null, no orphan `·`, no bare rail row (full survey: §5.3b). Provider omits its dedicated row outright when the show has no canonical identity, otherwise always renders (even all-`?`), dimming only when every provider is unchecked. Pinned omits its segment of that row when the show carries no pin: not an enrichment degrade, just absence. The `nextAiringEpisode` countdown ships under the same ROD-261 ticket but renders on the **chips row** (§4.4) instead, a live signal, not a stored snapshot |
